...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
	...
	//通过使用<d3dcompiler.h> / d3dcompiler.lib中的D3DCompile（），我们引入了对给定版本的d3dcompiler_XX.dll的依赖（参见D3DCOMPILER_DLL_A）
		 //如果您想使用此DX11示例代码但删除此依赖项，您可以：
		 // 1）编译一次，将编译后的着色器blob保存到文件或源代码中并将它们传递给CreateVertexShader（）/ CreatePixelShader（）[首选解决方案]
		 // 2）使用代码检测任何版本的DLL并从DLL中获取指向D3DCompile的指针。
		 //有关来源和详细信息，请参阅https://github.com/ocornut/imgui/pull/638。
		 ...
		 //创建顶点着色器
    {
        ...
        myD3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main", "vs_4_0", 0, 0, &g_pVertexShaderBlob, NULL);
    ...
    // Create the pixel shader
    {
        ...
        myD3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main", "ps_4_0", 0, 0, &g_pPixelShaderBlob, NULL);
