...
namespace ImGui
{
	...
	IMGUI_API void          ImageS(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1), const ImVec2& X = ImVec2(0, 0), const ImVec2& Y = ImVec2(0, 0), const ImVec4& tint_col = ImVec4(1, 1, 1, 1), const ImVec4& border_col = ImVec4(0, 0, 0, 0));
    IMGUI_API bool          ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0,0),  const ImVec2& uv1 = ImVec2(1,1), int frame_padding = -1, const ImVec4& bg_col = ImVec4(0,0,0,0), const ImVec4& tint_col = ImVec4(1,1,1,1));    // <0 frame_padding uses default frame padding settings. 0 for no padding
    IMGUI_API bool          Checkbox(const char* label, bool* v);
	IMGUI_API bool          CheckboxS(const char* label, bool* v);
...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_NoResize = 1 << 1,//使用右下方的手柄禁用用户调整大小
	ImGuiWindowFlags_NoMove = 1 << 2,//禁止用户移动窗口
	ImGuiWindowFlags_NoScrollbar = 1 << 3,//禁用滚动条（窗口仍然可以用鼠标滚动或以编程方式滚动）
	ImGuiWindowFlags_NoScrollWithMouse = 1 << 4,//禁止用户使用鼠标滚轮垂直滚动
	ImGuiWindowFlags_NoCollapse = 1 << 5,//通过双击禁用用户折叠窗口
	ImGuiWindowFlags_AlwaysAutoResize = 1 << 6,//每帧将每个窗口调整为其内容
	ImGuiWindowFlags_ShowBorders = 1 << 7,//显示窗口和项目周围的边框
	ImGuiWindowFlags_NoSavedSettings = 1 << 8,//从不在.ini文件中加载/保存设置
	ImGuiWindowFlags_NoInputs = 1 << 9,//禁用捕捉鼠标或键盘输入
	ImGuiWindowFlags_MenuBar = 1 << 10,//有一个菜单栏
	ImGuiWindowFlags_HorizontalScrollbar = 1 << 11,//允许水平滚动条出现（默认情况下关闭）。您可以使用SetNextWindowContentSize（ImVec2（width,0.0f））;在调用Begin（）之前指定宽度。在“水平滚动”部分中读取imgui_demo中的代码。
	ImGuiWindowFlags_NoFocusOnAppearing = 1 << 12,//从隐藏状态转换为可见状态时禁用焦点
	ImGuiWindowFlags_NoBringToFrontOnFocus = 1 << 13,//禁用在获得焦点时将窗口置于前面（例如,单击它或以编程方式对焦）
	ImGuiWindowFlags_AlwaysVerticalScrollbar = 1 << 14,//始终显示垂直滚动条（即使ContentSize.y <Size.y）
	ImGuiWindowFlags_AlwaysHorizontalScrollbar = 1 << 15,//始终显示水平滚动条（即使ContentSize.x <Size.x）
	ImGuiWindowFlags_AlwaysUseWindowPadding = 1 << 16,//确保没有边框的子窗口使用style.WindowPadding（默认情况下忽略非边框子窗口,因为更方便）
	// [内部]
	ImGuiWindowFlags_ChildWindow = 1 << 20,//不要使用！供BeginChild（）内部使用
	ImGuiWindowFlags_ChildWindowAutoFitX = 1 << 21,//不要使用！供BeginChild（）内部使用
	ImGuiWindowFlags_ChildWindowAutoFitY = 1 << 22,//不要使用！供BeginChild（）内部使用
	ImGuiWindowFlags_ComboBox = 1 << 23,//不要使用！供ComboBox内部使用（）
	ImGuiWindowFlags_Tooltip = 1 << 24,//不要使用！供BeginTooltip（）内部使用
	ImGuiWindowFlags_Popup = 1 << 25,//不要使用！供BeginPopup（）内部使用
	ImGuiWindowFlags_Modal = 1 << 26,//不要使用！供BeginPopupModal（）内部使用
	ImGuiWindowFlags_ChildMenu = 1 << 27 //不要使用！供BeginMenu（）内部使用
};
...
enum ImGuiInputTextFlags_
{
	...
	ImGuiInputTextFlags_CharsUppercase = 1 << 2,//将a..z变成A..Z
	ImGuiInputTextFlags_CharsNoBlank = 1 << 3,//过滤掉空格,制表符
	ImGuiInputTextFlags_AutoSelectAll = 1 << 4,//首次拍摄鼠标时选择整个文字
	ImGuiInputTextFlags_EnterReturnsTrue = 1 << 5,//按Enter键时返回'true'（与修改值时相反）
	ImGuiInputTextFlags_CallbackCompletion = 1 << 6,//按TAB时调用用户函数（完成处理）
	ImGuiInputTextFlags_CallbackHistory = 1 << 7,//按向上/向下箭头调用用户功能（用于历史记录处理）
	ImGuiInputTextFlags_CallbackAlways = 1 << 8,//每次调用用户函数。用户代码可以查询光标位置,修改文本缓冲区。
	ImGuiInputTextFlags_CallbackCharFilter = 1 << 9,//调用用户函数来过滤字符。修改data-> EventChar以替换/过滤输入,或返回1以丢弃字符。
	ImGuiInputTextFlags_AllowTabInput = 1 << 10,//按TAB键在文本字段中输入'\ t'字符
	ImGuiInputTextFlags_CtrlEnterForNewLine = 1 << 11,//在多行模式下,按Enter键退出编辑。按Ctrl + Enter添加新行（默认情况下,使用Enter添加新行）。
	ImGuiInputTextFlags_NoHorizontalScroll = 1 << 12,//水平跟随光标
	ImGuiInputTextFlags_AlwaysInsertMode = 1 << 13,//插入模式
	ImGuiInputTextFlags_ReadOnly = 1 << 14,//只读模式
	ImGuiInputTextFlags_Password = 1 << 15,//密码模式,将所有字符显示为'*'
	// [内部]
	ImGuiInputTextFlags_Multiline = 1 << 20 //供InputTextMultiline（）内部使用
};
...
enum ImGuiCol_
{
    ...
    ImGuiCol_WindowBg,              // 正常窗口背景
    ImGuiCol_ChildWindowBg,         //儿童窗口背景
    ImGuiCol_PopupBg,               // 弹出窗口，菜单，工具提示窗口的背景
    ImGuiCol_Border,
    ImGuiCol_BorderShadow,
    ImGuiCol_FrameBg,               // 复选框，单选按钮，情节，滑块，文本输入背景
...
//	ImGuiCol_PlotHistogramHovered,
//	ImGuiCol_TextSelectedBg,
//	ImGuiCol_ModalWindowDarkening,  // 当模态窗口处于活动状态时，整个屏幕变暗
...
struct ImGuiStyle
{
	float Alpha; //全局alpha适用于ImGui中的所有内容
	ImVec2 WindowPadding; //在窗口内填充
	ImVec2 WindowMinSize; //最小窗口大小
	float WindowRounding; //窗口半径四舍五入。设置为0.0f以具有矩形窗口
	ImVec2 WindowTitleAlign; //标题栏文本的对齐方式。对于左对齐，垂直居中，默认为（0.0f，0.5f）。
	float ChildWindowRounding; //子窗口的半径四舍五入。设置为0.0f以具有矩形窗口
	ImVec2 FramePadding; //在框架矩形内填充（大多数小部件使用）
	float FrameRounding; //框架角的半径四舍五入。设置为0.0f以具有矩形框（由大多数小部件使用）。
	ImVec2 ItemSpacing; //小部件/线条之间的水平和垂直间距
	ImVec2 ItemInnerSpacing; //组合小部件的元素之间的水平和垂直间距（例如滑块及其标签）
	ImVec2 TouchExtraPadding; //展开触摸位置不够准确的基于触摸的系统的反应边界框。不幸的是，我们不对小部件进行排序，因此重叠的优先级将始终给予第一个小部件。所以不要过多地增长！
	float IndentSpacing; //例如水平缩进进入树节点。通常==（FontSize + FramePadding.x * 2）。
	float ColumnsMinSpacing; //两列之间的最小水平间距
	float ScrollbarSize; //垂直滚动条的宽度，水平滚动条的高度
	float ScrollbarRounding; //滚动条的抓取角的半径
	float GrabMinSize; //滑块/滚动条的抓取框的最小宽度/高度。
	float GrabRounding; //获取圆角的半径。设置为0.0f以具有矩形滑块抓取。
	ImVec2 ButtonTextAlign; //当按钮大于文本时对齐按钮文本。水平+垂直居中默认为（0.5f，0.5f）。
	ImVec2 DisplayWindowPadding; //窗口位置被夹紧以在显示区域内可见至少这个量。仅涵盖常规窗户。
	ImVec2 DisplaySafeAreaPadding; //如果您看不到屏幕边缘（例如在电视上），请增加安全区域填充。包括弹出窗口/工具提示以及常规窗口。
	bool AntiAliasedLines; //在线条/边框上启用消除锯齿功能。如果您对CPU / GPU非常紧张，请禁用。
	bool AntiAliasedShapes; //在填充的形状上启用消除锯齿（圆角矩形，圆形等）
	float CurveTessellationTol; //细分容差。减少高度镶嵌的曲线（更高的质量，更多的多边形），增加以降低质量。
